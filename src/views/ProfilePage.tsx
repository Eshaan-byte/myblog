...
const ProfilePage = ({ username }: { username: string }) => {
  ...
  const fetchProfile = async () => {
    ...
    const [{ data: roleData }, { data: articleData }] = await Promise.all([
      (supabase.from("user_roles") as any).select("role").eq("user_id", profileData.user_id),
      articleQuery,
    ]);
    ...
    try {
      const [followersRes, followingRes, followCheck] = await Promise.all([
        (supabase.from("followers") as any).select("*", { count: "exact", head: true }).eq("author_id", profileData.user_id),
        (supabase.from("followers") as any).select("*", { count: "exact", head: true }).eq("follower_id", profileData.user_id),
        user && user.id !== profileData.user_id
          ? (supabase.from("followers") as any).select("id").eq("follower_id", user.id).eq("author_id", profileData.user_id).maybeSingle()
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        const { error } = await (supabase.from("followers") as any).upsert({ author_id: profile.user_id, follower_id: user.id }, { onConflict: "follower_id,author_id" });
        if (error) { toast.error(error.message); return; }
        setIsFollowing(true);
        setFollowerCount((c) => c + 1);
