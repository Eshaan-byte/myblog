...
const relatedBlogs = [
  { title: "US-Approved Spot Bitcoin ETFs Could Surpass Entire $50 Billion Crypto ETP Market", image: "/images/article-thumb-4.jpg", slug: "" },
  { title: "STX Price Prediction: After 126% Price Jump in December, What's in Store for 2024?", image: "/images/article-thumb-3.jpg", slug: "" },
  { title: "Over 65% of Crypto-Related Tweets and 84% of Conversations on Reddit Were Positive", image: "/images/article-thumb-2.jpg", slug: "" },
  { title: "Former FTX CEO Sam Bankman-Fried and Debtors Reach Settlement in Proceeding", image: "/images/article-thumb-5.jpg", slug: "" },
  { title: "Ethereum Summit Announces Major Protocol Upgrade for Q2 2024", image: "/images/article-card-2.jpg", slug: "" },
];
...
const ArticlePage = ({ slug }: { slug?: string }) => {
  ...
  useEffect(() => {
    ...
    if (cmsPost) {
      ...
      if (cmsPost.authorId) {
        (supabase.from("profiles") as any).select("username").eq("user_id", cmsPost.authorId).single()
          .then(({ data: p }: any) => { if (p) setAuthorUsername(p.username); });
      }
  ...
  const handleToggleLike = async () => {
    ...
    if (liked) {
      ...
      const { error } = await (supabase.from("likes") as any).insert({ article_id: article.id, user_id: user.id });
  ...
  const handleToggleSave = async () => {
    ...
    try {
      if (isSaved) {
        ...
        await (supabase.from("bookmarks") as any).insert({ article_id: article.id, user_id: user.id });
        setIsSaved(true);
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        const { error } = await (supabase.from("followers") as any).insert({ author_id: article.author_id, follower_id: user.id });
        if (error) { toast.error(error.message); return; }
        setIsFollowing(true);
  ...
  const displayRelated = relatedArticles.length > 0
    ? relatedArticles.map(a => ({ title: a.title, image: a.cover_image || "/images/article-thumb-4.jpg", slug: a.slug }))
    : relatedBlogs;
      ...
      <main className="max-w-[1320px] mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-8">
          <div>
            <ScrollReveal direction="scale" duration={0.9}>
              <LazyImage
                src={article.cover_image || "/images/article-hero-detail.jpg"}
                alt={article.title}
                    ...
                    <ScrollReveal key={comment.id} direction="up" delay={i * 0.06}>
                      <div className="glass-panel rounded-xl p-4 card-hover-glass">
                        <div className="flex items-start gap-3">
                          <LazyImage
                            src={comment.profile?.avatar_url || "/images/article-thumb-1.jpg"}
                ...
                <div className="space-y-2">
                  {displayRelated.map((blog, i) => (
                    <ScrollReveal key={i} delay={0.3 + i * 0.08} direction="right">
                      <Link
                        href={blog.slug ? `/article/${blog.slug}` : "/article"}
